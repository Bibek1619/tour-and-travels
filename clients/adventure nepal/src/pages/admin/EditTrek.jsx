...
import { getAllToursApi, updateTourApi } from "@/api/tourApi";
...
const EditTrek = () => {
  ...
  const handleSubmit = () => {
    ...
    // Add existing trek data (to prevent overwriting)
    formData.append("title", trek.title);
    formData.append("category", trek.category);
    if (trek.region) {
      formData.append("region", trek.region._id || trek.region);
    }
    ...
    <AdminLayout>
      <div className="p-8">
        <div className="max-w-4xl mx-auto">
          {/* Header */}
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-gray-900">Edit Trek Images</h1>
            <p className="text-gray-600 mt-1">Replace images for: <strong>{trek.title}</strong></p>
            {trek.region && (
              <p className="text-sm text-gray-500">Region: {trek.region.name || trek.region}</p>
            )}
          </div>
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <div
                  ...
                  className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center cursor-pointer hover:border-green-400 hover:bg-green-50 transition-all"
                >
